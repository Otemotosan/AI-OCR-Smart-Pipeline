...
def create_delivery_note():
    """納品書テストPDF"""
    filename = os.path.join(OUTPUT_DIR, "test_納品書_山田商事.pdf")
...
def create_order_form():
    """注文書テストPDF"""
    filename = os.path.join(OUTPUT_DIR, "test_注文書_ABC商会.pdf")
    c = canvas.Canvas(filename, pagesize=A4)
    c.setFont(FONT_NAME, 24)
...
def create_invoice():
    """請求書テストPDF"""
    filename = os.path.join(OUTPUT_DIR, "test_請求書_田中工業.pdf")
    c = canvas.Canvas(filename, pagesize=A4)
    c.setFont(FONT_NAME, 24)
...
def create_generic():
    ...
    filename = os.path.join(OUTPUT_DIR, "test_会議メモ_20250125.pdf")
    c = canvas.Canvas(filename, pagesize=A4)

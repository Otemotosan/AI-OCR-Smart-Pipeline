...
class OrderLineItem(BaseModel):
    ...
    item_name: str | None = Field(None, description="品名")
    quantity: int | None = Field(None, description="数量")
    unit: str | None = Field(None, description="単位")
    unit_price: int | None = Field(None, description="単価")
    amount: int | None = Field(None, description="金額")
...
class OrderFormV1(BaseModel):
    ...
    schema_version: str = Field(default="v1", frozen=True)
    document_type: str = Field(default="order_form", frozen=True)
